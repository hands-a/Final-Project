...
import { FaPlus, FaUsers, FaBook, FaDollarSign, FaEdit, FaTrash, FaLayerGroup } from 'react-icons/fa';
...
const AdminDashboard = () => {
  ...
  const stats = [
    { title: "Total Revenue", value: `$${totalRevenue.toFixed(2)}`, icon: <FaDollarSign />, color: "from-green-500 to-emerald-500" },
    { title: "Active Students", value: totalStudents, icon: <FaUsers />, color: "from-blue-500 to-cyan-500" },
    { title: "Total Courses", value: courses.length, icon: <FaBook />, color: "from-purple-500 to-pink-500" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-24 pb-20 px-4 md:px-8 lg:px-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Instructor Dashboard</h1>
          <p className="text-slate-400 text-sm md:text-base">Manage your courses and track performance.</p>
        </div>
        <Link 
          to="/admin/add-course" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12">
        {stats.map((stat, index) => (
          <div key={index} className="bg-[#13151d] p-5 md:p-6 rounded-2xl border border-white/5 flex items-center gap-4">
            <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-r ${stat.color} flex items-center justify-center text-white text-xl md:text-2xl shadow-lg`}>
              {stat.icon}
            </div>
            <div>
              <p className="text-slate-400 text-xs md:text-sm uppercase tracking-wider font-semibold">{stat.title}</p>
      ...
      <div className="bg-[#13151d] rounded-2xl border border-white/5 overflow-hidden">
        <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
          <h3 className="text-lg md:text-xl font-bold text-white">Your Courses</h3>
        ...
        {courses.length > 0 ? (
          <>
                      ...
                      <td className="p-5 font-bold text-white group-hover:text-purple-400 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded bg-slate-800 overflow-hidden shrink-0">
                            {course.image ? (
                                  <img src={typeof course.image === 'string' ? course.image : URL.createObjectURL(course.image)} alt="" className="w-full h-full object-cover" />
                            ) : (
                               <div className="w-full h-full flex items-center justify-center text-slate-500"><FaBook /></div>
                            )}
                          </div>
                          <span className="line-clamp-1">{course.title}</span>
                        </div>
                      </td>
                      <td className="p-5 font-mono text-green-400">${course.price}</td>
                      <td className="p-5 flex items-center gap-2">
                        <FaUsers className="text-slate-500" /> {course.students || 0}
                      </td>
                      <td className="p-5">
            ...
            <div className="md:hidden flex flex-col gap-4 p-4">
              {courses.map((course) => (
                <div key={course.id} className="bg-white/5 border border-white/5 rounded-xl p-4 flex flex-col gap-4">
                  <div className="flex items-start justify-between gap-4">
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                           ...
                           <span className="text-xs text-slate-400 bg-white/10 px-2 py-0.5 rounded mt-1 inline-block">{course.category}</span>
                        </div>
                     </div>
                     <span className="font-mono text-green-400 font-bold">${course.price}</span>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-slate-400 text-sm max-w-xs mb-6">Create your first course to start accepting students and earning revenue.</p>
            <Link 
              to="/admin/add-course" 
